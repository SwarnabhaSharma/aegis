...
| UI-Phase1 shell | console.css density + trust tokens + states; base.html IA nav + a11y; `_ui.html` macros; no backend change | 28 (partial) | **DONE 9deb906** — Phase2+ deferred, see §28 rows |
| UI-Phase2 overview | `/api/overview` real aggregation + dashboard Image2 density; test_overview_endpoint_empty; test_dashboard_renders → Overview | 28 (partial) | **DONE 9deb906** — Phase3 queue next |
| UI-Phase3 queue | q/sort/page on API + console; index.html density + pagination; New-via-API link (no fake create form) | 28 (partial) | **DONE 9deb906** — Phase4 detail next |
| UI-Phase4 detail | `GET /incidents/{id}/replay` merged timeline+transitions+approvals (policy/verification untimestamped, own cards); incident.html header + phase stepper + trust badges + replay; manifest kept; test_replay_merged_shape | 28 (partial) | **DONE 9deb906** — Phase5 agent feed next |
| UI-Phase5 agents | `/api/agents/activity` + agents.html + nav link; derived status, tool list per agent; no reasoning exposed | 28 (partial) | **DONE 9deb906** — Phase6 response next |
| UI-Phase6 response | response.html 6-step chain (recommend→evidence→policy→approval→execution→verification) + approve/deny wired to backend gate; approval/response_action/manifest/evidence added to console_response; A5 full text noted unpersisted; simulated-executor labeled; test_response_chain_renders | 28 (partial) | **DONE 9deb906** — Phase7 privacy/audit next |
| UI-Phase7 privacy/audit | privacy.html Data Access Log from real evidence (time/type/asset/purpose/classification + masked kinds); removed fake per-agent requested/released columns (records lack fields, audit holds truth); audit ?category=&actor= filters; test_privacy_log_and_audit_filter | 28 (partial) | **DONE 9deb906** — demo scenario next |
| UI-Phase8 demo + MVP close | docs/demo-scenario.md (offline slice RESOLVED trace, screens, real-vs-simulated, regression proof); full suite green; 13/13 routes 200 | 28, 30, 31 | **DONE 9deb906** — MVP complete, commit pending |
