...
| UI shell + design system (Phase1) | PRESENT | Image2-density metric cards, 8 trust badges, sev dots, skeleton/error/stale, focus-visible + skip link + reduced-motion, `_ui.html` macros; nav relabeled Overview; backend untouched; pytest offline green + 5 console pages 200 |
| Overview real-stats restyle (Phase2) | PRESENT | `GET /api/overview` (active/awaiting/investigating/success-pct/threats-enriched/top-assets/trend7/recent/agent-feed, partial-safe); dashboard metric cards + trend/type/assets + agent feed + TI + system; empty/error/offline states; fake metrics omitted |
| Queue search/sort/pagination (Phase3) | PRESENT | `GET /incidents` + console `/` share q/sort/order/page/limit; list shape unchanged; queue columns + pagination + empty states; test_incidents_search_and_page |
| Agent Activity global feed (Phase5) | PRESENT | `GET /api/agents/activity` + `/console/agents` page; role/status/tools/attempts derived, no CoT; test_agents_activity_empty |
| Ad-hoc KQL query + AI chat | DEFERRED (UI-Phase1) | no user query endpoint; evidence read-only until `POST /api/incidents/{id}/query` reviewed |
| TI / Assets / Reports / full Settings pages | DEFERRED (UI-Phase1) | no backend; TI widget only; integrations matrix would be fake |
...
| UI-Phase1 shell | console.css density + trust tokens + states; base.html IA nav + a11y; `_ui.html` macros; no backend change | 28 (partial) | **DONE uncommitted** — Phase2+ deferred, see §28 rows |
| UI-Phase2 overview | `/api/overview` real aggregation + dashboard Image2 density; test_overview_endpoint_empty; test_dashboard_renders → Overview | 28 (partial) | **DONE uncommitted** — Phase3 queue next |
| UI-Phase3 queue | q/sort/page on API + console; index.html density + pagination; New-via-API link (no fake create form) | 28 (partial) | **DONE uncommitted** — Phase4 detail next |
| UI-Phase4 detail | `GET /incidents/{id}/replay` merged timeline+transitions+approvals (policy/verification untimestamped, own cards); incident.html header + phase stepper + trust badges + replay; manifest kept; test_replay_merged_shape | 28 (partial) | **DONE uncommitted** — Phase5 agent feed next |
| UI-Phase5 agents | `/api/agents/activity` + agents.html + nav link; derived status, tool list per agent; no reasoning exposed | 28 (partial) | **DONE uncommitted** — Phase6 response next |
| UI-Phase6 response | response.html 6-step chain (recommend→evidence→policy→approval→execution→verification) + approve/deny wired to backend gate; approval/response_action/manifest/evidence added to console_response; A5 full text noted unpersisted; simulated-executor labeled; test_response_chain_renders | 28 (partial) | **DONE uncommitted** — Phase7 privacy/audit next |
| UI-Phase7 privacy/audit | privacy.html Data Access Log from real evidence (time/type/asset/purpose/classification + masked kinds); removed fake per-agent requested/released columns (records lack fields, audit holds truth); audit ?category=&actor= filters; test_privacy_log_and_audit_filter | 28 (partial) | **DONE uncommitted** — demo scenario next |
| UI-Phase8 demo + MVP close | docs/demo-scenario.md (offline slice RESOLVED trace, screens, real-vs-simulated, regression proof); full suite green; 13/13 routes 200 | 28, 30, 31 | **DONE uncommitted** — MVP complete, commit pending |
| Live-ES verify (2026-09-06) | `AEGIS_STORE=es run_slice --llm fake` → RESOLVED full 9-state trace, evidence 3, ALLOW, verify passed; cross-incident SHARED_INDICATOR edges live; 26 incidents in ES | 27, 14 | **DONE** — real-model run still needs llama-server |
| Live-model fixes (2026-09-06) | ES `add_record` JSON-safe coercion (raw TelemetryEvent broke ES persist); registry kwarg strip to schema_in (LLM-invented query/index args); `get_host_details` client-side agg (no keyword subfields); slice privacy_withheld dup-kwarg pop; integration test_add_record_coerces_unserializable | 8, 10, 18 | **DONE uncommitted** — live run then hits model truncation (server n_predict), needs longer predict length |
| Live-model RESOLVED (2026-09-06) | Ornith-1.0 + real telemetry + ES store → RESOLVED, A1-A5 ok, ALLOW isolate_host, verify passed; needed LLM_TEMPERATURE=0.6 (new knob, temp-0 greedy degenerates) + final-turn FINAL nudge (PROMPT_VERSION 2) + `_raw_result` strip in es_store | 7, 13, 21 | **DONE uncommitted** |
| Live TI (2026-09-06) | TI_PROVIDERS=local,abuseipdb,virustotal,otx; 3 providers enabled; 185.220.101.4 → malicious via AbuseIPDB | 8 | **DONE uncommitted** |
